...
def calculate_demographic_data(print_data=True):
    ...
    race_count=race_count=df['race'].value_counts()#.sort_index()
    ...
    #round(df[]/len(df)*100,2)

    rich_percentage = df[(df['hours-per-week']==min_work_hours) & (df['salary']=='>50K')].shape[0]/num_min_workers*100

    # What country has the highest percentage of people that earn >50K?
    
    #total_50K=df[df['salary']=='>50K'].count().iloc[0]
    highest_earning_country = (df.loc[df["salary"] == ">50K",
    "native-country"].value_counts() / df["native-country"].value_counts()).sort_values(ascending=False).index[0]
    ...
    top_IN_occupation=df.loc[(df['native-country']=='India') & (df['salary']=='>50K'),'occupation'].value_counts().sort_values(ascending=False).index[0]
